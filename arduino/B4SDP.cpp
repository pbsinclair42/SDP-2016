...
void motorForward(int motorNum, int motorPower) { //Makes Motor motorNum go forwards at a power of motorPower
  if (motorNum >= 0 and motorNum <= 5){
    ...
    Wire.beginTransmission(MOTOR_BOARD_ADDRESS); //open I2C communation to Motor Board.
...
void updateMotorPositions(int *positions) {
...
void printMotorPositions(int *positions) {
...
void restoreMotorPositions(int *positions){
  int i;
